...
qnnArch = [3, 1]  # architecture of the network, for example [2,3,2] or [4,1]
numTrainingPairs = 10  # number of training pairs
maxNumberSupervisedPairs = 10  # the functions starts with one supervised training pair and ends with maxNumberSupervisedPairs supervised training pairs
lda = 1  # lambda = 1/learning rate
ep = 0.01  # epsilon = step size
trainingRounds = 1000
gamma = -0.5  # negative number, weight of the graph part in the training cost
adjThreshold = 0.93  # state pairs (ij), which a fidelity over adjThreshold are close (adjecency matrix A_{ij}=1)
adjThreshold2 = 1  # set this to 1 or use it: state pairs (ij) , which a fidelity over adjThreshold2 are a bit close (adjecency matrix A_{ij}=0.5)
delta = 0  # set to 0 or use it: creates noise in the training pairs
shots = 30  # number of shots for mean values in the plot

makeLossListMean(qnnArch, numTrainingPairs, maxNumberSupervisedPairs, lda, ep, trainingRounds, gamma, adjThreshold,
                 adjThreshold2, delta, shots, "lineOutputRandomShuffled")

numTrainingPairs = 8  # number of training pairs
maxNumberSupervisedPairs = 8  # the functions starts with one supervised training pair and ends with maxNumberSupervisedPairs supervised training pairs
adjThreshold = 0.65

makeLossListMean(qnnArch, numTrainingPairs, maxNumberSupervisedPairs, lda, ep, trainingRounds, gamma, adjThreshold,
                 adjThreshold2, delta, shots, "connectedClustersRandomShuffled")

# the kind of the graph choose from: ADC, circleOutput, circleUnitary, classification, classificationSup, clustersRandom, clustersRandomShuffled, connectedClustersRandom, connectedClustersRandomShuffled, lineOutput, lineOutputRandom, lineUnitary, randomUnitary
